...
func TestInit_CommandConstruction(t *testing.T) {
	...
	exec := cmdtest.SetupCmdForTest(
		...
		func(f cmdutils.Factory) *cobra.Command {
			...
			return NewCmd(f, runCommandMatcher(t, "my-tofu", []string{
				...
				"-backend-config=username=testuser",
				"-backend-config=password=testtoken",
	...
	tc.MockUsers.EXPECT().CurrentUser(gomock.Any()).Return(&gitlab.User{Username: "testuser"}, nil, nil)
