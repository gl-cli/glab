...
func (f *DefaultFactory) HttpClient() (*gitlab.Client, error) {
	...
	c, err := api.NewClientWithCfg(repoHost, f.config, false, f.buildInfo.UserAgent())
	if err != nil {
		return nil, err
	}
...
func (f *DefaultFactory) resolveBaseRepoFromRemotes() (glrepo.Interface, error) {
	...
	ac, err := api.NewClientWithCfg(remotes[0].RepoHost(), f.config, false, f.buildInfo.UserAgent())
	if err != nil {
		return nil, err
	}
