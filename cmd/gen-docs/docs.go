...
func genRootCommandDocs(cmd *cobra.Command, basePath string) error {
	...
	// Write to index.md in the base path
	filename := filepath.Join(basePath, "index.md")
	if err := config.WriteFile(filename, out.Bytes(), 0o644); err != nil {
		return err
	}
...
func GenRootMarkdownCustom(cmd *cobra.Command, w io.Writer) error {
	...
	buf.WriteString("# GitLab CLI - `glab`" + "\n\n")
	buf.WriteString("![GLab](../assets/glab-logo.png)\n\n")
	...
	buf.WriteString("![command example](../assets/glabgettingstarted.gif)\n\n")
